/**
 * Optimization sweep v2: Stop Ticks × Target Ticks × BE Trigger
...
const STOP_TICKS = [160, 170, 180, 190, 200];
const TARGET_TICKS = [100, 125, 150, 175, 200];
const BE_TRIGGER = [30, 35, 40, 45, 50, 55, 60];
...
(async () => {
  console.log('=== Optimization v2: Stop Ticks × Target Ticks × BE Trigger ===');
  ...
  console.log(`Stop Ticks: ${STOP_TICKS.join(', ')}`);
  console.log(`Target Ticks: ${TARGET_TICKS.join(', ')}`);
  console.log(`BE Trigger: ${BE_TRIGGER.join(', ')}\n`);
  ...
  for (let i = 0; i < combos.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map((combo) => {
      ...
      return runOne(client, ind, label).catch(err => ({
        label, elapsed: '?', trades: 0, netProfit: 0, netProfitPct: 0,
        winRate: 0, profitFactor: 0, maxDD: 0, maxDDPct: 0, error: err.message,
      }));
    });
    ...
    if (i + BATCH_SIZE < combos.length) await sleep(BATCH_PAUSE_MS);
  }
