/**
 * Test VECTOR Pattern Strategy locally — reproduce job_32e0e381c17a11087346a846
 *
 * Runs 2 tests:
 *   1. HTF Timeframe = "60" (what the extension sent)
 *   2. HTF Timeframe = "2"  (what the TV chart shows)
 *
 * Both use Cont Min Slope Ticks = 10 (the chart default) for comparison.
 *
 * Start the server first:
 *   cd cloud-optimizer && DATABASE_URL="postgresql://neondb_owner:npg_Ye5EhdVPt7Bm@ep-weathered-sun-ae3in13i.c-2.us-east-2.aws.neon.tech/neondb?sslmode=require" node server.js
 *
 * Then run:
 *   node examples/test-vector-local.js
 */
...
// All default params from the job log — exact copy
function buildDefaultParams(htfTimeframe, contMinSlopeTicks) {
  return [
    ...
    { name: 'HTF Timeframe', index: 'in_2', type: 'dropdown', currentValue: htfTimeframe },
    ...
    { name: 'Cont Min Slope Ticks', index: 'in_72', type: 'float', currentValue: String(contMinSlopeTicks), isOptimized: true },
    // Properties
...
async function runJob(label, htfTimeframe, contMinSlopeTicks) {
  console.log(`\n=== ${label} ===`);
  console.log(`  HTF Timeframe: ${htfTimeframe}, Cont Min Slope Ticks: ${contMinSlopeTicks}\n`);
    ...
    parameters: [
      {
        ...
        dataType: 'float',
        defaultValue: String(contMinSlopeTicks),
        input: { mode: 'single', start: String(contMinSlopeTicks), end: String(contMinSlopeTicks), increment: '1' },
      },
    ],
    defaultParams: buildDefaultParams(htfTimeframe, contMinSlopeTicks),
    totalCombinations: 1,
  });
  ...
  // Cleanup
  try {
    await axios.post(`${API_BASE}/api/cloud/control`, { auth: makeAuth(), jobId, action: 'sync_complete' });
  } catch (e) { /* ignore */ }
}

(async () => {
  try {
    // Test 1: HTF = "60" (what the extension sent)
    await runJob('Test 1: HTF Timeframe = 60 (extension value)', '60', 10);

    // Test 2: HTF = "2" (what the TV chart shows)
    await runJob('Test 2: HTF Timeframe = 2 (TV chart value)', '2', 10);

    // Test 3: HTF = "2" with Cont Min Slope Ticks = 14 (exact chart config)
    await runJob('Test 3: HTF=2, Cont Min Slope=14 (exact chart match)', '2', 14);
