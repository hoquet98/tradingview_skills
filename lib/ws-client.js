...
async function fetchStrategyReport(scriptId, symbol, options = {}) {
  ...
  if (params && typeof params === 'object') {
    ...
    for (const [id, input] of Object.entries(indicator.inputs)) {
      if (input.name) nameToId[input.name.trim()] = id;
    }
    for (const [key, value] of Object.entries(params)) {
      const inputId = nameToId[key.trim()] || key;
      indicator.setOption(inputId, value);
    }
  }
...
async function fetchDeepBacktest(scriptId, symbol, options = {}) {
  ...
  // Apply parameter overrides if provided
  if (params && typeof params === 'object') {
    // Build name-to-id map for human-readable param names (trimmed)
    const nameToId = {};
    for (const [id, input] of Object.entries(indicator.inputs)) {
      if (input.name) nameToId[input.name.trim()] = id;
    }
